...
api_url = "http://api.speeds.everettdeleon.com/api/speeds/read"

def postData(Ip, name, download, upload, ping, timestamp):
    api_url = "http://api.speeds.everettdeleon.com/api/speeds/create"
...
def updateData(Ip, name, download, upload, ping, timestamp, id):
    api_url = f"http://api.speeds.everettdeleon.com/api/speeds/update/{id}"
    data = {"Ip": Ip, "name": name, "download": download, "upload": upload, "ping": ping, "timestamp":timestamp}
    print(data)
    response = requests.post(api_url, json=data)
