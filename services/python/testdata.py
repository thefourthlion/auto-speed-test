...
update_url = f"https://apispeeds.portkeylabs.net/api/speeds/update/{id}"
get_url = f"https://apispeeds.portkeylabs.net/api/speeds/read"
...
def updateTestData(hour, download, upload, ping):
    ...
    matching_entry = next(
        (entry for entry in existing_data if entry['Ip'] == host_name), None)
...
# You can set this to a specific number if you want the loop to stop after a certain count
max_iterations = None
...
while True:
    ...
    hostname_in_data = any(
        entry for entry in existing_data if entry['name'] == host_name)
