...
const Packages = () => {
    ...
    const getData = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:3025/api/packages/read');
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post('http://127.0.0.1:3025/api/packages/create', data);
    ...
    const deleteItem = async (id) => {
        try {
            const response = await axios.delete(`http://127.0.0.1:3025/api/packages/delete/${id}`);
            console.log(response.data)
            refreshPage()
        } catch (error) {
            console.error('Error:', error);
