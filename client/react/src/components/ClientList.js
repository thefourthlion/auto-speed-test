...
const ClientList = () => {
    ...
    const getpackageData = () => {
        fetch("http://192.168.0.66:4001/api/packages/read")
            .then((res) => res.json())
            .then((data) => {
                setpackages(data);
            })
    ...
    const getGroupData = () => {
        fetch("http://192.168.0.66:4001/api/groups/read")
            .then((res) => res.json())
            .then((data) => {
                setGroups(data);
            })
    ...
    const updateClientpackage = async (id) => {
        try {
            const response = await axios.post(`http://192.168.0.66:4001/api/speeds/update/${id}`, { package: chosenSpeed });
    ...
    const updateClientGroup = async (id) => {
        try {
            const response = await axios.post(`http://192.168.0.66:4001/api/speeds/update/${id}`, { group: chosenGroup });
            // console.log(response.data)
            refreshPage()
        } catch (error) {
            console.error('Error:', error);
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`http://192.168.0.66:4001/api/speeds/delete/${id}`);
